...
def f1(problem, pq, i):
    ...
    for x in pq:
        for j in range(size):
            if x.state == lst1[0][j+1]:
                x.value = x.path_cost + int(w) + int(lst1[i][j+1])  # f(n) = g(n) + h(n)
                break
...
def solPath(Problem, op):
    if op is None:
        print("NO SLN PATH FOUND")
        print("throughStates: 0")
        print("pathCost: 0")
    res1 = []
    cost = op.path_cost
    ...
    res1.append(op.state)
    print("slnPath: ", res1)
    print("throughStates", len(res1) + 1)
    print("expNodes: ", Problem.expNodes)
    print("pathCost: ", cost)
